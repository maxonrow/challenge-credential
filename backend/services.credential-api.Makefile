...
ROOT_PATH := $(shell pwd)
SERVICE_NAME := credential-api
SERVICE_PATH := $(ROOT_PATH)/services/$(SERVICE_NAME)
...
build: $(ROOT_PATH)/services.$(SERVICE_NAME).Dockerfile
	@docker build -t $(SERVICE_NAME):$(shell node -p "require('$(SERVICE_PATH)/package.json').version") \
		-f $(ROOT_PATH)/services.$(SERVICE_NAME).Dockerfile . && \
	docker tag $(SERVICE_NAME):$(shell node -p "require('$(SERVICE_PATH)/package.json').version") $(SERVICE_NAME)
